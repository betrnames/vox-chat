      ...
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            className="hidden lg:inline-flex items-center gap-1.5 text-xs font-mono text-muted-foreground hover:text-foreground transition-colors"
          >
          ...
          <VoiceCallTrigger
            className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/80 transition-colors disabled:opacity-60"
            idleLabel="Call now"
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-border/50 bg-background/95 backdrop-blur-xl px-5 py-4 space-y-1">
          ...
          <a
            ...
            className="block px-3 py-2.5 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors font-mono"
          >
            Call {VAPI_FREE_NUMBER_DISPLAY}
          </a>
          <VoiceCallTrigger
            onClick={() => setMenuOpen(false)}
        ...
        <div className="grid lg:grid-cols-[1fr_1fr] gap-12 lg:gap-24 items-center">
          ...
          <div className="text-center lg:text-left">
            ...
            <p className="text-muted-foreground text-lg max-w-lg leading-relaxed mb-8 mx-auto lg:mx-0">
              AI answers your phone, captures website leads, and gets you more{' '}
              <span className="inline-flex items-center gap-0.5 align-baseline">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className="w-3.5 h-3.5 text-review" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                ))}
              </span>{' '}
              positive reviews — 24/7.
            </p>
            {/* Trust strip */}
            ...
            <p className="mt-3 text-center lg:text-left text-sm text-muted-foreground">
              Or call the AI agent:{' '}
              <a
                href={VAPI_FREE_NUMBER_TEL}
                className="font-mono font-semibold text-foreground hover:text-primary transition-colors underline-offset-2 hover:underline"
              >
                {VAPI_FREE_NUMBER_DISPLAY}
              </a>
              <span className="text-muted-foreground/70"> — ask for a live person to reach Luis</span>
            </p>
          </div>
...
/* Live SMS “Try on your phone” (LiveReviewTry) stays off the public demos until Twilio is upgraded
   for custom copy. APIs remain: POST /api/reviews + /api/reviews-inbound — see docs/poc-reviews.md */
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-16 items-start">
          <div className="lg:sticky lg:top-24">
            ...
            {/* Phone agent CTA — best path for live transfer to Luis */}
            <div className="mb-8 rounded-2xl border border-voice/30 bg-voice/5 p-5 space-y-3">
              <p className="font-mono text-[11px] uppercase tracking-wider text-voice">Talk to the AI agent</p>
              <a
                href={VAPI_FREE_NUMBER_TEL}
                className="flex items-center gap-3 group"
              >
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-voice text-white shadow-md shadow-voice/25">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                  </svg>
                </span>
                <span>
                  <span className="block text-lg font-bold font-mono tracking-tight text-foreground group-hover:text-voice transition-colors">
                    {VAPI_FREE_NUMBER_DISPLAY}
                  </span>
                  <span className="block text-sm text-muted-foreground">
                    Call this number to speak to an agent — say “live person” to reach Luis
                  </span>
                </span>
              </a>
              <VoiceCallTrigger
                className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-lg border border-voice/40 text-sm font-semibold text-voice hover:bg-voice/10 transition-colors disabled:opacity-60"
                idleLabel="Or talk in browser"
                activeLabel="End call"
                connectingLabel="Connecting…"
              />
            </div>
